...
contract Pr000xy is ERC20, ERC20Detailed, Pr000xyInterface {
     ...
     */
    enum Condition {
        // nibble (1 hex character, 1/2 byte)
        NoMatch, // 0
     ...
     */
    constructor(address rewardsRegistry, bytes memory proxyInitCode) public ERC20Detailed("Pr000xy", "000", 0) {
     ...
     */
    function createProxy(bytes32 salt) external containsCaller(salt) returns (address proxy) {
    ...
    function claimProxy(
        ...
        _finalizeClaim(msg.sender, proxy, _getValue(leadingZeroBytes, totalZeroBytes));
    }
    ...
    function claimLatestProxy(
        ...
        require(proxyCount > 0, "No proxies found that correspond to the given zero byte arguments.");
    ...
    function makeOffer(
        ...
        address recipientFallback = (recipient == address(0) ? msg.sender : recipient);
        ...
        _offerLocations[offerID] = Location({exists: true, index: _offers.length});
     ...
     */
    function matchOffer(address proxy, uint256 offerID) external proxyExists(proxy) offerExists(offerID) {
        ...
        // burn tokens from the claiming address if applicable and emit an event.
        _finalizeClaim(offer.offerer, proxy, _getValue(leadingZeroBytes, totalZeroBytes));

        // pop the requested proxy out of the collection by index location.
        _popProxyFrom(leadingZeroBytes, totalZeroBytes, _proxyLocations[proxy].index);
     ...
     * @return The address of the new proxy.
     */
    function createAndMatch(bytes32 salt, uint256 offerID) external offerExists(offerID) containsCaller(salt) {
     ...
     */
    function batchCreate(bytes32[] calldata salts) external returns (address[] memory proxies) {
     ...
     */
    function batchCreateEfficient_H6KNX6() external {
        ...
        for (uint256 i; i < passedSaltSegments; i++) {
            ...
            assembly {
                // solhint-disable-line
                proxy := create2(
                    // call create2 and store output result.
                    ...
                    add(
                        // combine msg.sender and provided salt.
     ...
     */
    function batchCreateAndMatch(bytes32[] calldata salts, uint256[] calldata offerIDs) external {
        // declare variables.
        ...
        for (uint256 i; i < salts.length; i++) {
            ...
            if (hasOffer) {
                ...
                totalTokenValue = totalTokenValue + _processBatchDeploymentWithMatch(proxy, offer);
            } else {
     ...
     */
    function scheduleOfferExpiration(uint256 offerID) external offerExists(offerID) returns (uint256 expiration) {
        ...
        require(_offers[offerIndex].expiration == 0, "Offer has already been set to expire.");
    ...
    function cancelOffer(uint256 offerID) external offerExists(offerID) {
        ...
        require(_offers[offerIndex].expiration != 0, "Offer has not been set to expire.");
     ...
     */
    function findProxyCreationAddress(bytes32 salt) external view returns (address proxy) {
        // variable for checking if the current proxy is already deployed.
        bool proxyAlreadyCreated;
     ...
     */
    function matchesOffer(address proxy, uint256 offerID) external view returns (bool hasMatch) {
        require(_offerLocations[offerID].exists, "No offer found that corresponds to the given offer ID.");
        ...
        return ((offer.expiration == 0 || offer.expiration > now) &&
     ...
     */
    function getValue(uint256 leadingZeroBytes, uint256 totalZeroBytes) external view returns (uint256 value) {
     ...
     */
    function countProxiesAt(uint256 leadingZeroBytes, uint256 totalZeroBytes)
        external
        view
        returns (uint256 totalPertinentProxies)
    {
        ...
        )
    {
        ...
        return (offer.amount, offer.expiration, offer.searchSpace, offer.target, offer.offerer, offer.recipient);
    }
     ...
     */
    function getInitialProxyImplementation() external view returns (address implementation) {
     ...
     */
    function getProxyInitializationCode() external view returns (bytes memory initializationCode) {
        return _initCode;
    }
     ...
     */
    function getProxyInitializationCodeHash() external view returns (bytes32 initializationCodeHash) {
    ...
    function _isAdmin(address proxy) internal view returns (bool admin) {
        ...
        (bool success, bytes memory result) = proxy.staticcall(abi.encodeWithSignature("admin()"));
     ...
     * @return The address of the new proxy.
     */
    function _deployProxy(bytes32 salt, bytes memory initCodeWithParams) internal returns (address proxy) {
        // using inline assembly: load data and length of data, then call CREATE2.
        assembly {
            // solhint-disable-line
            let encodedData := add(0x20, initCodeWithParams)
            let encodedSize := mload(initCodeWithParams)
            proxy := create2(callvalue, encodedData, encodedSize, salt)
        }
    }
    ...
    function _processDeployment(address proxy) internal {
        ...
        _proxyLocations[proxy] = Location({exists: true, index: _proxies[leadingZeroBytes][totalZeroBytes].length});
     ...
     */
    function _processBatchDeployment(address proxy) internal returns (uint256 proxyTokenValue) {
        // calculate total number of leading and trailing zero bytes, respectively.
        (uint256 leadingZeroBytes, uint256 totalZeroBytes) = _getZeroBytes(proxy);
     ...
     * @return The number of additional tokens that will be minted.
     */
    function _processBatchDeploymentWithMatch(address proxy, Offer memory offer)
        internal
        returns (uint256 proxyTokenValue)
    {
     ...
     */
    function _finalizeDeployment(address proxy, uint256 proxyTokenValue) internal {
   ...
   */
    function _processOfferFulfillment(address proxy, Offer memory offer) internal {
     ...
     */
    function _validateAndFulfillExistingMatch(Offer memory offer, bytes32 salt)
        internal
        returns (bool willSkipDeployment)
    {
        ...
        if (
            offer.recipient == address(0) || (offer.expiration != 0 && offer.expiration <= now) // solhint-disable-line
        ) {
    ...
    function _transferProxy(
        ...
        AdminUpgradeabilityProxyInterface proxyInterface = (AdminUpgradeabilityProxyInterface(proxy));
     ...
     */
    function _popProxy(uint256 leadingZeroBytes, uint256 totalZeroBytes) internal returns (address proxy) {
        ...
        require(count > 0, "No proxy found that corresponds to the given arguments.");
    ...
    function _popProxyFrom(
        ...
        require(count > index, "No proxy found that corresponds to the given arguments.");
     ...
     */
    function _proxyCreationDryRun(bytes32 salt) internal view returns (address proxy, bool alreadyCreated) {
     ...
     * @return The reward size given the number of leading and zero bytes.
     */
    function _getValue(uint256 leadingZeroBytes, uint256 totalZeroBytes) internal view returns (uint256 value) {
    ...
    function _matchesOfferConstraints(
        ...
        for (uint256 i; i < p.length; i++) {
            ...
            leftNibbleSearchSpace = ((uint8(searchSpace[i]) - rightNibbleSearchSpace) / 16);
            ...
            if (!_nibbleConditionMet(leftNibbleProxy, caps[2 * i], leftNibbleSearchSpace, leftNibbleTarget)) {
            ...
            if (
                !_nibbleConditionMet(rightNibbleProxy, caps[2 * i + 1], rightNibbleSearchSpace, rightNibbleTarget) &&
                !_nibbleRelativeConditionMet(
                    rightNibbleProxy,
                    caps[2 * i + 1],
        ...
        returns (bool[40] memory characterIsCapitalized)
    {
        ...
        for (uint256 i; i < a.length; i++) {
            ...
            characterIsCapitalized[2 * i] = (leftNibbleAddress > 9 && leftNibbleHash > 7);
            characterIsCapitalized[2 * i + 1] = (rightNibbleAddress > 9 && rightNibbleHash > 7);
        }
    }
    ...
    function _nibbleRelativeConditionMet(
        ...
        if (searchSpace == uint8(Condition.MatchAgainstPriorCaseSensitive)) {
            return ((proxy == prior) && (capitalization == priorCapitalization));
        }

        if (searchSpace == uint8(Condition.MatchAgainstInitialCaseSensitive)) {
            return ((proxy == initial) && (capitalization == initialCapitalization));
        }
    }
    ...
    function _requireSuccessfulDeployment(address proxy) internal pure {
        ...
        require(proxy != address(0), "Failed to deploy an upgradeable proxy using the provided salt.");
    }
    ...
    function _requireOnlyOfferOriginator(uint256 index) internal view {
        ...
        require(_offers[index].offerer == msg.sender, "Only the originator of the offer may perform this operation.");
    }
     ...
     * nibble determined by the corresponding byte in searchSpace.
     */
    function _requireOfferValid(bytes20 searchSpace, address target) internal pure {
     ...
     */
    function _requireNoTargetUnlessNeeded(uint8 searchItem, uint8 target) internal pure {
        require(target == 0 || searchItem < 10, "Cannot specify target for relative or no-match conditions.");
    }
    ...
    function _requireNoCaseOnDigit(uint8 searchItem, uint8 target) internal pure {
        if (
            target < 10 &&
            (searchItem == uint8(Condition.MatchUpperCase) || searchItem == uint8(Condition.MatchLowerCase))
        ) {
     ...
     * @param target uint8 The nibble that designates what value to target.
     */
    function _requireNoCaseOnDigitRange(uint8 searchItem, uint8 target) internal pure {
        if (
            searchItem > 5 &&
     ...
     * @param target uint8 The nibble that designates what value to target.
     */
    function _requireRangeBoundsValid(uint8 searchItem, uint8 target) internal pure {
        if (
            searchItem > 3 &&
            ((target == 0 &&
    ...
    modifier proxyExists(address proxy) {
        ...
        require(_proxyLocations[proxy].exists, "No proxy found that corresponds to the given address.");
        _;
    }
